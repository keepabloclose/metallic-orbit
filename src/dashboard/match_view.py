...
def render_match_details(match_info, predictor):
        ...
        with c_ai2:
            ...
            st.markdown(
                ...
                <div style="background-color: #f0f2f6; padding: 20px; border-radius: 10px; text-align: center;">
                    <p style="margin: 0; color: #666; font-size: 0.9em; margin-bottom: 5px;">Predicción Exacta</p>
                    <h1 style="margin: 0; color: #333;">{pred_h_goals} - {pred_a_goals}</h1>
                </div>
                """, 
    ...
    # We need to compute these on the fly from match history
    ...
    def get_avg_stats(team, n_games):
        # Ensure we look for "Past" matches relative to match_date if provided, 
        # but here we just sorting by Date descending from entire history.
        # Problem: predictor.history might not have current match if it's future.
        # We need to filter where Date < match_info['Date'] if possible to replicate 'at that time',
        # but for simple 'recent form' just taking last N from history is fine.
        ...
        # Calculate separately for when they were Home vs Away, then combine? 
        # For simplicity, we just check "GoalsFor" logic
        ...
        for _, m in matches.iterrows():
            if m['HomeTeam'] == team:
                stats['Goles'] += m['FTHG']
                stats['Tiros'] += m.get('HST', 0) + m.get('HS', 0) # Approximation if total shots missing
    ...
    # Try to determine current season
    # If using football-data, 'Season' is usually e.g. '2425'
    # We find the max season present in the history for this league
    if league_code:
